...
Note: html_to_plain_text() is implemented via the html2text library, so the
output is markdown-flavored plain text: "**bold**", "_italic_",
"[text](url)", "<br>" becomes a markdown hard break ("  \n"), and the
output ends with a trailing blank line ("\n\n").
"""
import pytest
import sys
...
class TestHtmlToPlainText:
    """Tests for html_to_plain_text function (html2text-based)."""
    ...
    def test_br_tag_to_newline(self):
        """Test <br> tag conversion to a markdown hard break."""
        html = "Line1<br>Line2"
        result = html_to_plain_text(html)
        assert result == "Line1  \nLine2\n\n"
    ...
    def test_p_closing_to_newline(self):
        """Test that consecutive paragraphs are blank-line separated."""
    ...
    def test_multiple_entities(self):
        ...
        "<tag>" is parsed as an (unknown) HTML tag and dropped; the
        trailing &nbsp; survives as a space.
        """
        html = "<tag> & &nbsp;"
        result = html_to_plain_text(html)
        assert result == "&  \n\n"
    ...
    def test_preserves_single_newlines(self):
        """Test that single line breaks are preserved (as hard breaks)."""
    ...
    def test_bold_and_italic_tags(self):
        """Test <b>, <strong>, <i>, <em> become markdown emphasis."""
        html = "<b>Bold</b> and <strong>strong</strong> and <i>italic</i> and <em>emphasis</em>"
        result = html_to_plain_text(html)
        assert result == "**Bold** and **strong** and _italic_ and _emphasis_\n\n"

    def test_link_tag(self):
        """Test <a> tags become markdown links."""
        html = '<a href="http://example.com">Link text</a>'
        result = html_to_plain_text(html)
        assert result == "[Link text](http://example.com)\n\n"
